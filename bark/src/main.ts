...
async function createFeeAccount(payer: Keypair): Promise<PublicKey> {
  try {
    ...
    const createFeeAccountInstruction = SystemProgram.createAccount({
      ...
      space: 165, // Adjust the space based on BARK requirements
...
async function harvestWithheldTokensToMint(mintAccount: PublicKey, feeAccount: string) {
  try {
    const feeAccountKey = new PublicKey(feeAccount);  // Convert the fee account string to PublicKey
    ...
    if (withheldAmount > 0) {
      const transferSignature = await transferCheckedWithFee(
        connection,
        payerWallet,
        feeAccountKey,  // Use the PublicKey instance
        mintAccount,
        mintAuthority,
        withheldAmount,
        3,
